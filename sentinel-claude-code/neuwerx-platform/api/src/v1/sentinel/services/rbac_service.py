...
from ..models import User, Role, Group, Permission, UserRole, UserGroup, GroupRole, RolePermission
...
class RBACService:
    ...
    async def _resolve_user_permissions(self, user: User) -> List[str]:
        ...
        # 6. SECURITY: Verify that users with roles actually have permissions configured
        # Fail secure - deny access if roles exist but have no permissions
        if len(resolved_roles) > 0 and len(all_scopes) == 0:
            role_names = [role.name for role in resolved_roles]
            logger.error(f"SECURITY ALERT: User {user.id} has roles {role_names} but no permissions are configured in the database. This indicates incomplete RBAC setup. Denying access.")
            raise Exception(f"Access denied: User has roles {role_names} but no permissions are configured. Please contact administrator to properly configure RBAC permissions.")
        
        # 7. Apply permission conflict resolution
    ...
    def _permissions_to_scopes(self, permissions: List[Permission]) -> List[str]:
        ...
        for permission in permissions:
            # Use the permission name directly as scope - this is the intended design
            # The permission.name field contains the full scope string (e.g., 'tenant:read', 'user:admin')
            if permission.name:
                scopes.append(permission.name)
            else:
                # Fallback to building scope from resource_type and actions if name is not set
