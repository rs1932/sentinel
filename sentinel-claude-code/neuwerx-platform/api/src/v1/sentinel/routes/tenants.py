...
async def list_tenants(
    ...
    # TENANT ISOLATION: Determine tenant filtering based on user's tenant
    # Platform users (PLATFORM tenant) can see all tenants
    # Tenant admins can only see their own tenant and sub-tenants
    current_user_tenant_id = None
    if current_user.tenant_code != "PLATFORM":
        current_user_tenant_id = current_user.tenant_id
    
    return await service.list_tenants(query, current_user_tenant_id)
