...
class TestTenantAPI:
    ...
    def test_create_tenant_duplicate_code(self, client: TestClient):
        tenant_data = {
            "name": "First Company",
            "code": "DUP-001",
            "type": "root",
    ...
    def test_get_tenant_by_code(self, client: TestClient):
        tenant_data = {
        ...
        get_response = client.get(f"/api/v1/tenants/code/{tenant_data['code']}")
        assert get_response.status_code == 200
        data = get_response.json()
        assert data["code"] == tenant_data["code"]
        assert data["name"] == tenant_data["name"]
    ...
    def test_list_tenants_with_filters(self, client: TestClient):
        tenant_data = {
        ...
        response = client.get("/api/v1/tenants/?name=Filter")
    ...
    def test_update_tenant(self, client: TestClient):
        ...
        update_response = client.patch(f"/api/v1/tenants/{tenant_id}", json=update_data)
    ...
    def test_create_sub_tenant(self, client: TestClient):
        parent_data = {
            "name": "Parent Company",
            "code": "PARENT-001",
            "type": "root",
        ...
        parent_response = client.post("/api/v1/tenants/", json=parent_data)
        ...
        sub_response = client.post(f"/api/v1/tenants/{parent_id}/sub-tenants", json=sub_tenant_data)
    ...
    def test_activate_deactivate_tenant(self, client: TestClient):
        tenant_data = {
        ...
        deactivate_response = client.post(f"/api/v1/tenants/{tenant_id}/deactivate")
        assert deactivate_response.status_code == 200
        assert deactivate_response.json()["is_active"] is False
        
        activate_response = client.post(f"/api/v1/tenants/{tenant_id}/activate")
        assert activate_response.status_code == 200
        assert activate_response.json()["is_active"] is True
    
    def test_get_tenant_hierarchy(self, client: TestClient):
        parent_data = {
        ...
        hierarchy_response = client.get(f"/api/v1/tenants/{sub_id}/hierarchy")
